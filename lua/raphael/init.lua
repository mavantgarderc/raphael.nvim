...
function M.apply(theme, from_manual)
	...
	-- Clear highlights before applying new theme
	vim.cmd("hi clear")
	...
	if not ok then
		vim.notify(
			"raphael: failed to apply theme '" .. tostring(theme) .. "': " .. tostring(err),
			vim.log.levels.ERROR
		)
		if themes.is_available(M.config.default_theme) then
			-- Clear again before fallback
			vim.cmd("hi clear")
			if vim.fn.exists("syntax_on") then
				vim.cmd("syntax reset")
			end
...
function M.show_status()
	...
	local saved_info = M.state.saved
			and M.state.saved ~= M.state.current
