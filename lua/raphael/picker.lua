...
local function render(opts)
	...
	if #(state_ref.bookmarks or {}) > 0 then
		local bookmark_icon = ICON_GROUP_EXP -- Always expanded for simplicity, or add collapse if desired
		...
		for _, t in ipairs(state_ref.bookmarks) do
			if search_query == "" or (t:lower():find(search_query:lower(), 1, true)) then
				...
				local b = " " -- No need for bookmark icon here since it's the section
	...
	-- Then the rest: if is_display_grouped etc.
